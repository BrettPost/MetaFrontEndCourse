        ...
        <>
            <form onSubmit={handleSubmit}>
                ...
                <div>
                    ...
                    <select id="scheduleTime" name="scheduleTime" onChange={handleChange} value={formData.scheduleDate}>
            ...
            {reservation.map((r, index) => (
                <div className='reservation' key={index}>
                    <h6>Name: {r.firstName} {r.lastName}</h6>
                    <p>Date: {r.scheduleDate}</p>
                    <p>Time: {r.scheduleTime}</p>
                    <p>Guests: {r.numOfGuests}</p>
                    <p>Occasion: {r.occasion}</p>
                </div>
            ))}           
        </>
    );
